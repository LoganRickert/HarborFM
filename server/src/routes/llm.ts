...
import { readSettings, redactError } from './settings.js';
...
export async function llmRoutes(app: FastifyInstance) {
  app.get('/api/llm/available', {
    ...
    const available =
      ...
      (settings.llm_provider === 'openai' && !!settings.openai_api_key?.trim());
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const model = (settings.model || '').trim() || (provider === 'openai' ? OPENAI_DEFAULT_MODEL : 'llama3.2:latest');

      if (provider === 'none') {
        return reply.status(400).send({ error: 'No LLM provider configured. Set one in Settings.' });
      }
      ...
      if (provider === 'ollama') {
        ...
        try {
          ...
          if (!res.ok) {
            ...
            return reply.status(502).send({ error: redactError(errText || `Ollama returned ${res.status}`) });
          }
      ...
      if (provider === 'openai') {
        ...
        if (!apiKey) {
          return reply.status(400).send({ error: 'OpenAI API key is not set in Settings.' });
        }
        try {
