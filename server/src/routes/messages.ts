  ...
  app.get(
    '/api/messages',
    {
      ...
      schema: {
        ...
        description: 'Paginated list of contact form messages. Admins see all; others see only messages for their podcasts.',
        querystring: {
          type: 'object',
          properties: {
            page: { type: 'string' },
    ...
    async (request) => {
      ...
      const totalCount = (countParams.length > 0 ? countQuery.get(...countParams) : countQuery.get()) as { count: number };
      ...
      const rows = db.prepare(selectStr).all(...listParams) as ContactMessageRow[];
