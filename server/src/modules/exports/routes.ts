...
function getExport(
  ...
  const row = db.prepare("SELECT * FROM exports WHERE id = ?").get(exportId) as
    | (Record<string, unknown> & { podcast_id: string })
    | undefined;
  if (!row) return null;
...
export async function exportRoutes(app: FastifyInstance) {
  app.post(
    ...
    async (request, reply) => {
      ...
      const publicBaseUrl =
        (data as Record<string, unknown>).public_base_url ?? null;
      const configEnc = buildConfigEnc(
        mode,
      ...
      const row = db
        ...
        .get(expId) as Record<string, unknown>;
  ...
  app.patch(
    ...
    async (request, reply) => {
      ...
      db.prepare(`UPDATE exports SET ${fields.join(", ")} WHERE id = ?`).run(
        ...values,
      );
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      const row = db
        .prepare("SELECT * FROM export_runs WHERE id = ?")
        .get(id) as
        | (Record<string, unknown> & { podcast_id: string })
        | undefined;
