...
export async function sitemapRoutes(app: FastifyInstance) {
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      if (settings.public_feeds_enabled) {
        const rows = db
          .prepare(
            "SELECT id, slug FROM podcasts WHERE COALESCE(unlisted, 0) = 0 ORDER BY created_at ASC",
          )
          .all() as {
          id: string;
          slug: string;
  ...
  // Admin: clear sitemap cache (deletes all sitemap files so next request regenerates).
