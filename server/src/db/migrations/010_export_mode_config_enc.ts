 ...
 */
export const up = (db: { exec: (sql: string) => void; prepare: (sql: string) => { all: () => { name: string }[] } }) => {
  const cols = (db.prepare('PRAGMA table_info(exports)').all() as { name: string }[]).map((r) => r.name);
