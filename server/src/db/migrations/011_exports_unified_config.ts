...
export const up = (db: Database) => {
  const tableInfo = db.prepare("PRAGMA table_info(exports)").all() as {
    name: string;
  }[];
  ...
  if (hasLegacyColumns) {
    const rows = db
      .prepare(
        `SELECT id, mode, config_enc, bucket, prefix, region, endpoint_url,
    ...
    const updateStmt = db.prepare(
      "UPDATE exports SET config_enc = ? WHERE id = ?",
    );
    ...
    for (const row of rows) {
      ...
      if (bucketEnc && isEncryptedSecret(bucketEnc)) {
        ...
        region =
          regionEnc && isEncryptedSecret(regionEnc)
            ? decryptSecret(regionEnc, AAD)
            : String(row.region ?? "");
        endpoint =
          endpointEnc && isEncryptedSecret(endpointEnc)
            ? decryptSecret(endpointEnc, AAD)
            : (row.endpoint_url as string)?.trim() || null;
      } else {
      ...
      if (
        accessKeyEnc &&
        secretEnc &&
        isEncryptedSecret(accessKeyEnc) &&
        isEncryptedSecret(secretEnc)
      ) {
  ...
  const tableInfoAfter = db.prepare("PRAGMA table_info(exports)").all() as {
    name: string;
  }[];
