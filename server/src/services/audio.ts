...
import {
  contentTypeFromAudioPath,
  mimeFromAudioFormatName,
} from "../utils/audio.js";
import {
  FFPROBE_PATH,
  FFMPEG_PATH,
...
function prepareOutputPath(outputPath: string, allowedBaseDir: string): void {
  ...
  if (
    resolvedOut === resolvedTmp ||
    resolvedOut.startsWith(resolvedTmp + sep)
  ) {
...
export async function probeAudio(
  ...
  if (durationSec <= 0) {
    try {
      const { stdout: durationOut } = await exec(
        FFPROBE_PATH,
        [
          "-v",
          "error",
  ...
  const mime = mimeFromAudioFormatName(formatName);
  return {
    durationSec,
    format: formatName,
    sizeBytes: stat.size,
...
export async function normalizeUploadToMp3OrWav(
  ...
  if (ext === "mp3" || ext === "wav") {
    return {
      path: assertPathUnder(inputPath, allowedBaseDir),
      mime: contentTypeFromAudioPath(`x.${ext}`),
      ext,
    };
  }
  ...
  const outPath = join(
    dirname(safeIn),
    basename(safeIn, extname(safeIn)) + ".mp3",
  );
  ensureDir(dirname(outPath));
  await exec(
    FFMPEG_PATH,
    ["-i", safeIn, "-acodec", "libmp3lame", "-b:a", "128k", "-y", outPath],
    { maxBuffer: 1024 * 1024 },
  );
  try {
    unlinkSync(safeIn);
  ...
  return {
    path: assertPathUnder(outPath, allowedBaseDir),
    mime: "audio/mpeg",
    ext: "mp3",
  };
}
 ...
 * Output path: same directory, same base name, extension from WAVEFORM_EXTENSION.
 * Input path must be under allowedBaseDir; output is validated the same way.
 */
export async function generateWaveformFile(
  audioPath: string,
  allowedBaseDir: string,
): Promise<string> {
  ...
  const outPath = join(
    dirname(safeIn),
    basename(safeIn, extname(safeIn)) + WAVEFORM_EXTENSION,
  );
...
export async function removeSilenceFromWav(
  ...
  if (silencePeriods.length === 0) {
    ...
    await exec(
      FFMPEG_PATH,
      ["-i", safeSource, "-acodec", "pcm_s16le", "-ar", "44100", "-y", safeOut],
      { maxBuffer: 1024 * 1024 },
    );
    return safeOut;
  }
