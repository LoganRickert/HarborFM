...
export async function testSftpAccess(config: SftpConfig): Promise<{ ok: boolean; error?: string }> {
...
async function getSftpFile(sftp: SftpClient, remotePath: string): Promise<Buffer | null> {
  try {
...
export async function deployPodcastToSftp(
  ...
  try {
    ...
    if (config.private_key?.trim()) {
      ...
      return { uploaded: 0, skipped: 0, errors: ['Provide either password or private_key'] };
    }
    await sftp.connect(connectOpts);
