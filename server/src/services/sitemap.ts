...
import { assertPathUnder, assertResolvedPathUnder, sitemapDir, sitemapIndexDir } from './paths.js';

const SITEMAP_FILENAME = 'sitemap.xml';
const SITEMAP_INDEX_FILENAME = 'index.xml';
...
`;
  if (entry.lastmod) out += `    <lastmod>${escapeXml(entry.lastmod)}</lastmod>
`;
  if (entry.changefreq) out += `    <changefreq>${entry.changefreq}</changefreq>
`;
  if (entry.priority != null) out += `    <priority>${Math.min(1, Math.max(0, entry.priority)).toFixed(1)}</priority>
`;
  out += `  </url>
`;
  return out;
...
/**
 * Static sitemap: root, login, register, privacy, terms. Feed pages are in per-podcast sitemaps only.
 */
export function generateStaticSitemapXml(baseUrl: string): string {
  const now = toLastmod(new Date());
  const entries: SitemapUrlEntry[] = [
    { loc: loc(baseUrl, '/'), lastmod: now, changefreq: 'weekly', priority: 1.0 },
    { loc: loc(baseUrl, '/login'), lastmod: now, changefreq: 'monthly', priority: 0.5 },
    { loc: loc(baseUrl, '/register'), lastmod: now, changefreq: 'monthly', priority: 0.5 },
    { loc: loc(baseUrl, '/privacy'), lastmod: now, changefreq: 'yearly', priority: 0.3 },
    { loc: loc(baseUrl, '/terms'), lastmod: now, changefreq: 'yearly', priority: 0.3 },
    { loc: loc(baseUrl, '/contact'), lastmod: now, changefreq: 'monthly', priority: 0.3 },
  ];
  let out = `<?xml version="1.0" encoding="UTF-8"?>
 ...
 */
export function generatePodcastSitemapXml(podcastId: string, baseUrl: string): string {
  const podcast = db.prepare('SELECT id, slug, updated_at FROM podcasts WHERE id = ?').get(podcastId) as
    | { id: string; slug: string; updated_at: string }
    | undefined;
  if (!podcast) throw new Error('Podcast not found');
  const slugEnc = encodeURIComponent(podcast.slug);
  const feedLastmod = podcast.updated_at ? toLastmod(new Date(podcast.updated_at)) : toLastmod(new Date());
  const entries: SitemapUrlEntry[] = [
    {
  ...
  const episodes = db
    ...
    .all(podcastId) as { slug: string | null; publish_at: string | null; updated_at: string }[];
 ...
 */
export function getCachedSitemapIfFresh(podcastId: string, maxAgeMs: number): string | null {
  const dir = sitemapDir(podcastId);
  const path = join(dir, SITEMAP_FILENAME);
  assertResolvedPathUnder(path, dir);
