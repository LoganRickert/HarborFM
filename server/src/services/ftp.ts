...
import { FTP_CLIENT_TIMEOUT_MS, RSS_FEED_FILENAME } from "../config.js";
...
export async function testFtpAccess(
  config: FtpConfig,
): Promise<{ ok: boolean; error?: string }> {
  const client = new Client(FTP_CLIENT_TIMEOUT_MS, FTP_CLIENT_OPTIONS);
  try {
    await client.access({
      host: config.host,
      port: config.port,
  ...
  try {
    ...
    const upload = async (
      ...
      await client.uploadFrom(
        Readable.from(Buffer.from(hash, "utf8")),
        full + MD5_SUFFIX,
      );
      uploaded += 1;
    };
