...
export async function getObjectETag(config: S3Config, key: string): Promise<string | null> {
  const client = createS3Client(config);
  try {
    const res = await client.send(
      new HeadObjectCommand({ Bucket: config.bucket, Key: fullKey(config, key) })
    );
    const etag = res.ETag;
...
function s3ErrorMessage(err: unknown): string {
  ...
  const s = String(err);
  const out = s && s !== '[object Object]' ? s : 'S3 request failed (check server logs)';
  return out === 'Unknown' ? 'S3 request failed (check server terminal for details)' : out;
}

export async function testS3Access(config: S3Config): Promise<{ ok: boolean; error?: string }> {
  try {
    const client = createS3Client(config);
    ...
    console.error(
      ...
      e?.name ?? 'Error',
      e?.message ?? '',
      e?.$metadata?.httpStatusCode != null ? `(HTTP ${e.$metadata.httpStatusCode})` : ''
    );
...
export async function uploadFile(
  ...
  await client.send(
    new PutObjectCommand({
      ...
      Body: body,
      ContentType: contentType ?? (key.endsWith('.xml') ? 'application/xml' : 'audio/mpeg'),
    })
  );
}
...
export async function deployPodcastToS3(
  ...
  const existingFeedETag = await getObjectETag(config, 'feed.xml');
