...
function decryptConfigEnc(exp: Record<string, unknown>): Record<string, unknown> {
 ...
 */
export function getDecryptedConfigFromEnc(exp: Record<string, unknown>): ExportConfigDecrypted {
...
export function buildConfigEnc(mode: ExportMode, data: Record<string, unknown>): string {
...
const CONFIG_KEYS = new Set([
  ...
  'host', 'port', 'username', 'password', 'path', 'secure', 'private_key', 'url', 'api_url', 'api_key', 'gateway_url', 'share', 'domain',
]);
...
export function mergeAndEncryptConfig(
  ...
  for (const [k, v] of Object.entries(update)) {
    if (v !== undefined && CONFIG_KEYS.has(k)) (merged as Record<string, unknown>)[k] = v;
  }
  if (merged.path !== undefined) (merged as Record<string, unknown>).path = normalizePath(merged.path as string);
 ...
 */
export function getExportPathPrefix(exp: Record<string, unknown>): string | null {
  const mode = (exp.mode as ExportMode) || 'S3';
  try {
    const raw = decryptConfigEnc(exp);
    if (mode === 'S3') {
