...
export type AttemptContext = 'auth_login' | 'setup';

const THRESHOLD = 3; // ban on > 3 failures (i.e. 4th failure triggers ban)
const BAN_MINUTES = 10;
...
export function getIpBan(ip: string, context: AttemptContext): { banned: boolean; retryAfterSec: number } {
  const row = db
    .prepare(
      `
      SELECT
    ...
    .get(ip, context) as { banned_until: string; retry_after_sec: number } | undefined;
...
export function recordFailureAndMaybeBan(
  ...
  const attemptedEmail = meta?.attemptedEmail ? String(meta.attemptedEmail).trim().toLowerCase() : null;
  const userAgent = meta?.userAgent ? String(meta.userAgent).trim() : null;
  db.prepare('INSERT INTO login_attempts (ip, context, attempted_email, user_agent) VALUES (?, ?, ?, ?)').run(
    ip,
    context,
    attemptedEmail,
    userAgent
  ...
  const row = db
    ...
    .get(ip, context, `-${WINDOW_MINUTES} minutes`) as { count: number };
  ...
  `
  ).run(ip, context, `+${BAN_MINUTES} minutes`);

  // Compute retry-after from DB for consistency.
  const ban = getIpBan(ip, context);
  return { bannedNow: true, retryAfterSec: ban.retryAfterSec || BAN_MINUTES * 60, failuresInWindow: failures };
}
...
export function clearFailures(ip: string, context: AttemptContext): void {
  ...
  db.prepare('DELETE FROM login_attempts WHERE ip = ? AND context = ?').run(ip, context);
}
