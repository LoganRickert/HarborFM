...
import { probeAudioDurationSec } from "./probe.js";
...
export type SegmentRemakeTools = {
  ffmpegPath: string;
  ffprobePath: string;
};
...
async function renderSeekConcatLane(opts: {
  ...
  const { sourcePath, clips, probedMs, ffmpegPath } = opts;
  const sorted = [...clips]
    .map((c) => ({
      entry: c.entry,
...
export async function remakeMixFromMultitrackDir(
  ...
  outWavPath: string,
  tools: SegmentRemakeTools,
): Promise<{ durationSec: number }> {
  const { ffmpegPath, ffprobePath } = tools;
  ...
  for (const u of usable) {
    ...
    try {
      const durationSec = await probeAudioDurationSec(u.path, ffprobePath);
      probedMsByPath.set(u.path, Math.round(durationSec * 1000));
    } catch {
  ...
  const durationSec = await probeAudioDurationSec(safeOut, ffprobePath);
  return { durationSec };
}
